...
void getSensorData()
{
  ...
  float hTmp,tTmp;
  hTmp = dht.readHumidity();
  tTmp = dht.readTemperature();

  if( isnan( hTmp ) || isnan( tTmp ) )
  {
...
void loop ( void ) {
  ...
  getSensorData();
	server.handleClient();
  digitalWrite ( led, 1 );
  updatedht();// upload data to Server
  delay(Timing);
  digitalWrite ( led, 0 );
...
void updatedht()
{
    ...
    else
    {
        ...
        char sendStr[100]={};
        //sprintf(sendStr,"%d, %d,%.2f",(int)temperature,(int)humidity,(float)Irms*Rated_Voltage);
